...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-6">
      <a href="{{ route('addquestions') }}" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i> Add
        Questions</a>
    </div>
    <div class="col-md-6">
    </div>
      ...
      <table class="table table-bordered table-hover table-striped ar-datatable display nowrap" style="width: 100%;">
        ...
        <tbody>
          ...
          <tr>
            <td>{{ $sl }}</td>
            <td>
              <a href="{{ route('edtquestions', array('id' => $cat->id)) }}"><i
                  class="fa fa-pencil-square-o fa-2x base-green"></i></a>
              <a href="{{ route('delquestions', array('id' => $cat->id)) }}"
                onclick="return confirm('Sure To Delete This Category ?');"><i
                  class="fa fa-trash-o fa-2x base-red"></i></a>
              @if($cat->status == 1)
              <a href="{{ route('acInac') }}?id={{ $cat->id }}&val=2&tab=question_text">
                <i class="fa fa-check-circle-o base-green fa-2x" aria-hidden="true"></i>
              </a>
              @endif
              @if($cat->status == 2)
              <a href="{{ route('acInac') }}?id={{ $cat->id }}&val=1&tab=question_text">
