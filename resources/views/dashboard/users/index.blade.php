            ...
            @if(Auth::user()->user_type == 1 || Auth::user()->user_type == 4)
              <tr>
                <td>
                  ...
                  @endif
                   <a href="{{ route('sub_users_list', array('id' => $user->id))}}"> 
                ...
                <td>{{ ucfirst($user->member_company) }}<br/>{{isset($user->startupprogramme)  && isset($user->startupprogramme->programme->name) ? '( '.ucfirst($user->startupprogramme->programme->name).(' )'):''}}</td>
                <td>{{ ucfirst($user->contact_name) }}</td>
                <td>{{ $user->email_id }}</td>
                <td>{{ $user->contact_no }}, {{ $user->mobile }}</td>
                <td>{{ $user->city }}, {{ $user->country }}</td>
                <td>{{ $user->memberBusiness }}</td>
