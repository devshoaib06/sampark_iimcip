...
@section('content')
    ...
    @if (isset($checkIndustries) && !empty($checkIndustries) && count($checkIndustries) )
      @foreach($checkIndustries as $industry)
	{{--@php
                if (isset($industry->memberInfo) && !empty($industry->memberInfo)) {
                    $memberInfo = $industry->memberInfo;
                }
            @endphp
	--}}            
            <div class="col-sm-12">
                <div class="postCard">
                    <div class="postWrap">
                        <div class="postTop">
							...
							{{--@if(isset($industry->postIndistries) && !empty($industry->postIndistries) && count($industry->postIndistries))
                                <ul >
                                    @foreach($industry->postIndistries as $pi)
                                ...
                                @endif
                                @if(isset($industry->postCategories) && !empty($post->postCategories) && count($post->postCategories))
						   ...
						   <div class="postTopRight">
							@php
                                    ...
                                    $resultFav =DB::table('post_favourates')->where('post_id', '=', $industry->pmID)->where('member_id', '=', Auth::user()->id)->count();
							...
							@endphp
                                  ...
                                  <input type="hidden" name="post_hidden_{{$industry->pmID}}" id="post_hidden_{{$industry->pmID}}" value="{{$favourate}}">
                               
                                    <span class="post_fav_{{$industry->pmID}}"><a onclick="addFavourate({{$industry->pmID}},{{Auth::user()->id}})"> 
                                        @if($favourate==0)
                                ...
                                @if($industry->member_id == Auth::user()->id)
                                    <a href="{{ route('front.user.editpost', array('post_id' => $industry->pmID)) }}"><i class="fas fa-edit"></i></a>
                                @endif
						   </div>
                        </div>
                        @php 
                            $profileImage = $defaultImage = $authUserImage = asset('public/front_end/images/profile-pic.png');
                            if(isset($industry) && !empty($industry) && $industry->image != null) {
                                $profileImage = asset('public/uploads/user_images/thumb/'. $industry->image);
                            }
                        ...
                        <div class="postUser">
							...
							<div class="userInfo">
                                <h3>
                                    ...
                                    <a href="{{ route('front.user.memberProfile', array('member_company' => $industry->slug)) }}">{{ $industry->contact_name }}</a>
								
                                    @endif
                                    <span>{{ date('F d, Y', strtotime($industry->created_at)) }}</span>
									
                                </h3>
                                @if(isset($industry) && !empty($industry)) 
                                    <h4>
								<a href="{{ route('front.user.account') }}?search={{ $industry->member_company }}">{{ $industry->member_company }}</a>
                        ...
                        <div class="postTitle">
						<h2>@if($industry->is_bookmarked==1)
                                        <img src="{{asset('public/front_end/images/bookmark.png')}}" width="30px">
						@endif{{ $industry->post_title }}</h2>
                        </div>
                       <div class="postDetails">
						@php
                                $phtml = html_entity_decode($industry->post_info, ENT_QUOTES);
                                $pnohtml = strip_tags($phtml);
                            @endphp
                            @if(strlen($pnohtml) > 450)
                                <div id="mincontent_{{ $industry->id }}">
                                    <p>
                                        {!! str_limit($pnohtml, '450', '...') !!}
                                        <a href="javascript:void(0);" class="redm" id="redm_{{ $industry->id }}">View Details</a>
                                    </p>
                                </div>
                                <div id="fullcontent_{{ $industry->id }}" style="display: none;">
                                    <p>
                                        {!! html_entity_decode($industry->post_info, ENT_QUOTES) !!}
                                    </p>
                                </div>
                            @else
                                <p>{!! html_entity_decode($industry->post_info, ENT_QUOTES) !!}</p>
                            @endif

                            @if(!empty($industry->video_link))

                                 @php

                                    $video_url = $industry->video_link;
                                    ...
                                    $industry->video_link =getYoutubeEmbedUrl($industry->video_link);

                                @endphp

                                {{--<a onclick ="openVideoModal('{{$industry->video_link}}')"><img src="{{$thumbnail}}" class="pitch"></a>--}}
								<a onclick ="openVideoModal('{{$industry->video_link}}')"></a>

                            @endif
                        </div>
						
						@if(isset($industry->pmImage))
						<div class="postImages">
						
						<img src="/public/front_end/images/{{$industry->pmImage}}">
						...
						{{-- <div class="postFooter">
                            <ul>
                                <li class="commentLink">
                                    ...
                                    <span id="commCount_{{ $industry->id }}">
                        ...
                        <div class="postCommentWrap">
                            <div class="postComment"> <!-- COMMENT :: Level one add comment -->
                              <div class="post-comment-area">
                                ...
                                <input type="text" id="post_comment_{{ $industry->id }}-0" class="form-control input-comment" placeholder="Add a comment...">
                                <input type="button" id="addCommentBTN_{{ $industry->id }}-0" data-postid="{{ $industry->id }}" data-postpid="0" class="addCommentBTN btn-comment" value="Add comment">               </div>
                                
                                <div class="comment-attachment">
                                 <label for="post_image_{{ $industry->id }}-0">Add Images</label>
                                 <input type="file" id="post_image_{{ $industry->id }}-0"  class="form-control input-comment add-img" onchange="getComboA(this,'{{ $industry->id }}-0')" accept="image/x-png,image/gif,image/jpeg" multiple="true">
                                 <p id="error_image_{{ $industry->id }}-0" class="err-msg" style="color: red;"></p>
                                 <label for="post_video_{{ $industry->id }}-0">Add Video</label>
                                 <input type="text" id="post_video_{{ $industry->id }}-0"  class="form-control input-comment input-url" placeholder="Enter Youtube or Vimeo link" >
                            ...
                            <div class="preview-wrap">
                                    <div class="img_preview_{{ $industry->id }}-0" id="img_preview_{{ $post->id }}-0">
                                    </div>
                            ...
                            <div class="commentList">
                                @if(!empty($industry->postReplies) && count($industry->postReplies))
                                <ul>
                                    <li><div id="addCommentBox_{{ $post->id }}-0"></div></li>
                                    @foreach($industry->postReplies as $fpr)
                                    <li>
                                        <div class="commentWrapOuter">
                                            ...
                                            <div class="addCommentWrap"> <!-- REPLY :: Level two add comment -->
                                                <div class="addComment">
                                                    <img src="{{ $authUserImage }}" class="img-fluid">
                                                    <input type="text" id="post_comment_{{ $industry->id }}-{{ $fpr->id }}" class="form-control input-comment" placeholder="Add a comment...">
                                                    <input type="button" id="addCommentBTN_{{ $industry->id }}-{{ $fpr->id }}" data-postid="{{ $industry->id }}" data-postpid="{{ $fpr->id }}" class="addCommentBTN add-comment" value="Add comment">
                                                </div>
                                                <div class="comment-attachment">
                                                    <label for="post_image_{{ $industry->id }}-{{ $fpr->id }}">Add Images</label>
                                                    <input type="file" id="post_image_{{ $industry->id }}-{{ $fpr->id }}"  class="form-control input-comment add-img" onchange="getComboA(this,'{{ $industry->id }}-{{ $fpr->id }}')" accept="image/x-png,image/gif,image/jpeg" multiple="true" onchange="getComboA(this)">

                                                    <p id="error_image_{{ $industry->id }}-{{ $fpr->id }}" style="color: red;"></p>

                                                    <label for="post_video_{{ $industry->id }}-{{ $fpr->id }}">Add Video</label>
                                                    <input type="text" id="post_video_{{ $industry->id }}-{{ $fpr->id }}"  class="form-control input-comment input-url" placeholder="Enter Youtube or Vimeo link" >
                                                    
                                                </div>

                                                <div class="preview-wrap">
                                                    <div class="img_preview_{{ $industry->id }}-{{ $fpr->id }}" id="img_preview_{{ $industry->id }}-{{ $fpr->id }}">
                                                    </div>
                                                    <!-- <div class="video-preview">
                                                        <video class="vid-preview" width="80" height="40"></video>
                                                    </div> -->
                                        ...
                                        @if(isset($fpr->childReplies) && count($fpr->childReplies))
                                        <ul class="replyList">
                                            <li><div id="addCommentBox_{{ $industry->id }}-{{ $fpr->id }}"></div></li>
                                ...
                                <ul> <!-- NO FIRST COMMENT :: it works when no comment - only for first time -->
                                    <li><div id="addCommentBox_{{ $industry->id }}-0"></div></li>
                                </ul>
                                @endif
                            </div>
                        </div>
        ...
        <div class="col-sm-12 mt-2">
            @if(isset($checkIndustries) && count($checkIndustries)) {{ $checkIndustries->appends(request()->query())->links() }} @endif
        </div>
    ...
    <div class="col-sm-12">
        <div class="postCard">
            <div class="postWrap">
                <div class="postTop">
                    <p>Sorry! No industry category found.</p>
                </div>
            </div>
        </div>
    </div>
