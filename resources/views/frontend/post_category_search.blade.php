...
@if(isset($checkIndustries))	
 
	
	
	@include('frontend.industry_category_search')
 ...
 @include('frontend.notification_page')
...
{{--@if(Session::has('msg') && Session::has('msg_class'))
    ...
    @if (isset($checkPosts) && !empty($checkPosts) && count($checkPosts) )
        @foreach($checkPosts as $post)
                    ...
                    <div class="postWrap">
                        <div class="postTop">
							...
							@php
                                    ...
                                    $resultFav =DB::table('post_favourates')->where('post_id', '=', $post->pmID)->where('member_id', '=', Auth::user()->id)->count();
							...
							@endphp
                                  ...
                                  <input type="hidden" name="post_hidden_{{$post->pmID}}" id="post_hidden_{{$post->pmID}}" value="{{$favourate}}">
                               
                                    <span class="post_fav_{{$post->pmID}}"><a onclick="addFavourate({{$post->pmID}},{{Auth::user()->id}})"> 
                                        @if($favourate==0)
                                ...
                                @if($post->member_id == Auth::user()->id)
                                    <a href="{{ route('front.user.editpost', array('post_id' => $post->pmID)) }}"><i class="fas fa-edit"></i></a>
                        ...
                        @php 
                            ...
                            if(isset($post) && !empty($post) && $post->image != null) {
                                $profileImage = asset('public/uploads/user_images/thumb/'. $post->image);
                            }
                            ...
                            <div class="userImg">
							 ...
							 <div class="userInfo">
                                <h3>
                                    ...
                                    <a href="{{ route('front.user.memberProfile', array('member_company' => $post->slug)) }}">{{ $post->contact_name }}</a>
                                ...
                                @if(isset($post) && !empty($post)) 
                                    <h4>
								<a href="{{ route('front.user.account') }}?search={{ $post->member_company }}">{{ $post->member_company }}</a>
                       ...
                       <div class="postDetails">
						@php
                            ...
                            @if(!empty($post->video_link))
								...
								<a onclick ="openVideoModal('{{$post->video_link}}')"></a>

                            @endif
                        </div>
						
						@if(isset($post->pmImage))
						<div class="postImages">
						
						<img src="/public/front_end/images/{{$post->pmImage}}">
        ...
        <div class="col-sm-12 mt-2">
            @if(isset($checkPosts) && count($checkPosts)) {{ $checkPosts->appends(request()->query())->links() }} @endif
        </div>
    ...
    <div class="col-sm-12">
        <div class="postCard">
            <div class="postWrap">
                <div class="postTop">
                    <p>Sorry! No post category found.</p>
                </div>
            </div>
        </div>
    </div>
