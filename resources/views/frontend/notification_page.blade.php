...
@section('content')
    ...
    @if (isset($notification) && !empty($notification) && count($notification) )
      @foreach($notification as $notify)
	{{--@php
                if (isset($notify->memberInfo) && !empty($notify->memberInfo)) {
                    $memberInfo = $notify->memberInfo;
                }
            @endphp
	--}}            
            <div class="col-sm-12">
                <div class="postCard">
                    <div class="postWrap">
						...
						{{--@if(isset($notify->postIndistries) && !empty($notify->postIndistries) && count($notify->postIndistries))
                                <ul >
                                    @foreach($notify->postIndistries as $pi)
                                        @if(isset($pi->industryInfo) && !empty($pi->industryInfo) && $pi->industryInfo->industry_category != '')
                                            <li>
                                                <a href="{{ route('front.user.account') }}?notify={{ base64_encode($pi->industryInfo->id) }}">{{ $pi->industryInfo->industry_category }}</a>
                                            </li>
                                        @endif
                                    @endforeach
                                ...
                                @if(isset($notify->postCategories) && !empty($notify->postCategories) && count($notify->postCategories))
                                
                                    @foreach($notify->postCategories as $pc)
						...
						@endif--}}
						   ...
						   <div class="postTopRight">
							@php
                                    ...
                                    $resultFav =DB::table('post_favourates')->where('post_id', '=', $notify->pmID)->where('member_id', '=', Auth::user()->id)->count();
							...
							@endphp
                                  ...
                                  <input type="hidden" name="post_hidden_{{$notify->pmID}}" id="post_hidden_{{$notify->pmID}}" value="{{$favourate}}">
                               
                                    <span class="post_fav_{{$notify->pmID}}"><a onclick="addFavourate({{$notify->pmID}},{{Auth::user()->id}})"> 
                                        @if($favourate==0)
                                ...
                                @if($notify->member_id == Auth::user()->id)
                                    <a href="{{ route('front.user.editpost', array('post_id' => $notify->pmID)) }}"><i class="fas fa-edit"></i></a>
                        ...
                        @php 
                            ...
                            if(isset($notify) && !empty($notify) && $notify->image != null) {
                                $profileImage = asset('public/uploads/user_images/thumb/'. $notify->image);
                            }
                        ...
                        <div class="postUser">
                            <div class="userImg">
								...
								@if(isset($notify->industryInfo))	
								<p>
								<a href="{{ route('front.user.account') }}?notify={{ base64_encode($notify->industryId) }}">{{$notify->industryInfo}}</a>
								</p>
								@endif
							...
							<div class="userInfo">
                                <h3>
                                    ...
                                    <a href="{{ route('front.user.memberProfile', array('member_company' => $notify->slug)) }}">{{ $notify->contact_name }}</a>
								
                                    @endif
                                    <span>{{ date('F d, Y', strtotime($notify->created_at)) }}</span>
									
                                </h3>
                                @if(isset($notify) && !empty($notify)) 
                                    <h4>
								<a href="{{ route('front.user.account') }}?search={{ $notify->member_company }}">{{ $notify->member_company }}</a>
                        ...
                        <div class="postTitle">
						<h2>@if($notify->is_bookmarked==1)
                                        <img src="{{asset('public/front_end/images/bookmark.png')}}" width="30px">
						@endif{{ $notify->post_title }}</h2>
                        </div>
                       <div class="postDetails">
						@php
                                $phtml = html_entity_decode($notify->post_info, ENT_QUOTES);
                                $pnohtml = strip_tags($phtml);
                            @endphp
                            @if(strlen($pnohtml) > 450)
                                <div id="mincontent_{{ $notify->id }}">
                                    <p>
                                        {!! str_limit($pnohtml, '450', '...') !!}
                                        <a href="javascript:void(0);" class="redm" id="redm_{{ $notify->id }}">View Details</a>
                                    </p>
                                </div>
                                <div id="fullcontent_{{ $notify->id }}" style="display: none;">
                                    <p>
                                        {!! html_entity_decode($notify->post_info, ENT_QUOTES) !!}
                                    </p>
                                </div>
                            @else
                                <p>{!! html_entity_decode($notify->post_info, ENT_QUOTES) !!}</p>
                            @endif

                            @if(!empty($notify->video_link))

                                 @php

                                    $video_url = $notify->video_link;
                                    ...
                                    $notify->video_link =getYoutubeEmbedUrl($notify->video_link);

                                @endphp

                                <a onclick ="openVideoModal('{{$notify->video_link}}')"><img src="{{$thumbnail}}" class="pitch"></a>
								<a onclick ="openVideoModal('{{$notify->video_link}}')"></a>

                            @endif
                        </div>
						
						@if(isset($notify->pmImage))
						<div class="postImages">
						
						<img src="/public/front_end/images/{{$notify->pmImage}}" style="height: 150px; width: 250px;">
						...
						{{--<div class="postFooter">
                            <ul>
                                <li class="commentLink">
                                    ...
                                    <span id="commCount_{{ $notify->pmID }}">
									
									@if(isset($notify->reply_text))
									{{ count(array($notify->reply_text)) }}@endif
                                        @if(count(array($notify->reply_text)) > 0)
                                            Comments
                                        @else
                                            Comment
									@endif
                        ...
                        <div class="postCommentWrap">
                            <div class="postComment"> <!-- COMMENT :: Level one add comment -->
                              <div class="post-comment-area">
                                ...
                                <input type="text" id="post_comment_{{ $notify->pmID }}-0" class="form-control input-comment" placeholder="Add a comment...">
                                <input type="button" id="addCommentBTN_{{ $notify->pmID }}-0" data-postid="{{ $notify->pmID }}" data-postpid="0" class="addCommentBTN btn-comment" value="Add comment">               </div>
                                
                                <div class="comment-attachment">
                                 <label for="post_image_{{ $notify->pmID }}-0">Add Images</label>
                                 <input type="file" id="post_image_{{ $notify->pmID }}-0"  class="form-control input-comment add-img" onchange="getComboA(this,'{{ $notify->pmID }}-0')" accept="image/x-png,image/gif,image/jpeg" multiple="true">
                                 <p id="error_image_{{ $notify->pmID }}-0" class="err-msg" style="color: red;"></p>
                                 <label for="post_video_{{ $notify->pmID }}-0">Add Video</label>
                                 <input type="text" id="post_video_{{ $notify->pmID }}-0"  class="form-control input-comment input-url" placeholder="Enter Youtube or Vimeo link" >
                            ...
                            <div class="preview-wrap">
                                    <div class="img_preview_{{ $notify->pmID }}-0" id="img_preview_{{ $post->pmID }}-0">
                                    </div>
                            ...
                            <div class="commentList">
                                @if(!empty($notify->postReplies) && count($notify->postReplies))
                                <ul>
                                    <li><div id="addCommentBox_{{ $post->id }}-0"></div></li>
                                    @foreach($notify->postReplies as $fpr)
                                    <li>
                                        <div class="commentWrapOuter">
                                            ...
                                            <div class="addCommentWrap"> <!-- REPLY :: Level two add comment -->
                                                <div class="addComment">
                                                    <img src="{{ $authUserImage }}" class="img-fluid">
                                                    <input type="text" id="post_comment_{{ $notify->pmID }}-{{ $fpr->id }}" class="form-control input-comment" placeholder="Add a comment...">
                                                    <input type="button" id="addCommentBTN_{{ $notify->pmID }}-{{ $fpr->id }}" data-postid="{{ $notify->pmID }}" data-postpid="{{ $fpr->id }}" class="addCommentBTN add-comment" value="Add comment">
                                                </div>
                                                <div class="comment-attachment">
                                                    <label for="post_image_{{ $notify->pmID }}-{{ $fpr->id }}">Add Images</label>
                                                    <input type="file" id="post_image_{{ $notify->pmID }}-{{ $fpr->id }}"  class="form-control input-comment add-img" onchange="getComboA(this,'{{ $notify->id }}-{{ $fpr->id }}')" accept="image/x-png,image/gif,image/jpeg" multiple="true" onchange="getComboA(this)">

                                                    <p id="error_image_{{ $notify->pmID }}-{{ $fpr->id }}" style="color: red;"></p>

                                                    <label for="post_video_{{ $notify->pmID }}-{{ $fpr->id }}">Add Video</label>
                                                    <input type="text" id="post_video_{{ $notify->pmID }}-{{ $fpr->id }}"  class="form-control input-comment input-url" placeholder="Enter Youtube or Vimeo link" >
                                                    
                                                </div>

                                                <div class="preview-wrap">
                                                    <div class="img_preview_{{ $notify->pmID }}-{{ $fpr->id }}" id="img_preview_{{ $notify->pmID }}-{{ $fpr->id }}">
                                                    </div>
                                                    <!-- <div class="video-preview">
                                                        <video class="vid-preview" width="80" height="40"></video>
                                                    </div> -->
                                        ...
                                        @if(isset($fpr->childReplies) && count($fpr->childReplies))
                                        <ul class="replyList">
                                            <li><div id="addCommentBox_{{ $notify->pmID }}-{{ $fpr->id }}"></div></li>
                                ...
                                <ul> <!-- NO FIRST COMMENT :: it works when no comment - only for first time -->
                                    <li><div id="addCommentBox_{{ $notify->pmID }}-0"></div></li>
                                </ul>
                                @endif
                            </div>
                        </div>
        ...
        <div class="col-sm-12 mt-2">
            @if(isset($notification) && count($notification)) {{ $notification->appends(request()->query())->links() }} @endif
        </div>
    ...
    <div class="col-sm-12">
        <div class="postCard">
            <div class="postWrap">
                <div class="postTop">
                    <p>Sorry! No notify category found.</p>
                </div>
            </div>
        </div>
    </div>
