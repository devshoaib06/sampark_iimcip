...
@section('content')
    ...
    <div class="col-sm-12">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="financial_year" name="financial_year"
                                        value="{{ $orderpipe->financial_year }}">
                                </div>
                            </div>
                                    ...
                                    <select name="product_id[]" class="form-control indusCatIds" multiple="multiple">
                                        ...
                                        @if (isset($allProducts) && count($allProducts))
                                            @foreach ($allProducts as $c)
                                                <option value="{{ $c->id }}"
                                                    {{ $c->id == $orderpipe->product_id ? 'selected' : '' }}>{{ $c->caption }}
                                                </option>
                                            @endforeach
                                        @endif
                                    </select>
        ...
        <div class="mt-3">
            <a href="{{ route('front.user.addorderpipe') }}" class="btn btn-secondary mr-2"><i
