...
<div>
    ...
    {{-- <div class="card card-body bg-light p-4"> --}}
      ...
      <div class="postCard manage-wrap">
                    ...
                    <select name="financial_year" required class="form-control">
                        @if (isset($finYear) && count($finYear) > 0)
                            @foreach ($finYear as $finY)
                                <option value="{{ $finY->id }}" {{ $finY->id == $finance->financial_year ? 'selected': ''}}>{{ $finY->display_year }}</option>
                            @endforeach
                        @endif
                    </select>
            </div>
        ...
        <div class="mt-3">
            <a href="{{route('startup.addcust',[$finance->startup_id])}}" class="btn btn-secondary mr-2"><i class="fa fa-arrow-left"></i> Cancel</a>
