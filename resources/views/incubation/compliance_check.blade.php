...
@section('content')
    <div class="row">
        @if (Session::has('msg') && Session::has('msg_class'))
            <div class="col-sm-12">
                <div class="postCard">
                    <div class="{{ Session::get('msg_class') }}" style="margin-bottom: 0;" data-dismiss="alert"
                        aria-label="close">
                        {{ Session::get('msg') }}
                    </div>
                </div>
    ...
    <div class="col-lg-12 fin-report">
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                @foreach ($compliance as $key => $list)
                    <tr>

                        <td>{{ $key + 1 }}</td>
                        <td>{{ $list->getFinancialYear['display_year'] }}</td>
                        <td>{{ $list->audited_financials }}</td>
                        <td>{{ $list->gst_compliance }}</td>
                        @if($userType == 4)
                        <td>
                            <a class="btn btn-primary" href="{{ route('startup.compliancechecks.edit', $list->id) }}"><i
    ...
    <div class="col-sm-12">
        <div class="postCard manage-wrap">
            <div class="postWrap">
                <div class="pwdbox">
                    <h3>Add Compliance</h3>
                    <form name="frm_pfupd" id="frm_pfupd" action="{{ route('startup.addcomplact', [$startUpId]) }}"
    ...
    @push('page_js')
        ...
        <script type="text/javascript">
            ...
            function goDelete(id) {
                ...
                if (isConfirm) {
                    window.location.href = "{{ url('compliancechecks/destroy') }}/" + id;
                }
            }
        </script>
    @endpush
