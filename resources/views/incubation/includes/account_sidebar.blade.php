...
<div class="leftPanel pn-ProductNav_Wrapper">
    <nav id="pnProductNav" class="pn-ProductNav dragscroll">
        <div id="pnProductNavContents" class="pn-ProductNav_Contents">
            <a href="{{ route('dashboard', array('uid' => Auth::user()->id)) }}"  target="_blank" aria-selected="true">Portfolio Management</a>
            <span id="pnIndicator" class="pn-ProductNav_Indicator"></span>
        </div>
    </nav>
</div>
...
<div class="leftPanel pn-ProductNav_Wrapper res-height"  style=" 
    ...
    <nav id="pnProductNav" class="pn-ProductNav dragscroll">
		...
		<a href="{{ route('front.user.account') }}@if(isset($_GET['post']) && $_GET['post'] != '')?post={{ $_GET['post'] }}@endif" @if(!isset($_GET['industry'])) aria-selected="true" @endif >All Industries</a>
