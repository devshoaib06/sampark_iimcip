...
public function deleteMentor($id) {
	   ...
	   $res = DB::table('users')->where('id', $id)->delete();
        if( $res ) {
            return back()->with('msg_class', 'alert alert-success')
            ->with('msg', 'Mentor Deleted Succesfully.');
        } else {
            return back()->with('msg_class', 'alert alert-danger')
            ->with('msg', 'Something Went Wrong.');
        }
 ...
 public function createMentor() {
    ...
    public function saveMentor(Request $request) {
		...
		$Users->company = trim($request->input('company'));
		$Users->preferred_industry = trim($request->input('preferred_industry'));
		$Users->experience_years = trim($request->input('experience_years'));
		$Users->functional_expertise = trim($request->input('functional_expertise'));
		$Users->address = trim($request->input('address'));
		$Users->investment_interests = trim($request->input('investment_interests'));
		$Users->startup_stage_preference = trim($request->input('startup_stage_preference'));
		$Users->startups_mentored = trim($request->input('startups_mentored'));
      ...
      $file->move($destinationPath,$file->getClientOriginalName());
        ...
        if( $Users->save() ) {
            ...
            return back()->with('msg_class', 'alert alert-success')
            ->with('msg', 'New Mentor Created Succesfully.');
	...
	public function addInvitations($id){
		
		$User = Users::where('id', '=', $id)->first();
		 $sub='Welcome to Our Mentorship Programme as a Mentor';

                $emailData=array();
                $emailData['name']=trim($User->first_name);
                $emailData['email_id']=trim($User->email_id);
				$emailData['password']=Hash::make(Str::random(10));
                ...
                <table width="700" border="0" align="center" cellpadding="0" cellspacing="0">
                  ...
                  <tr>
                   ...
                   <p>We cordially welcome you to our Mentorship Programme and thank you for sharing your valuable time to guide the startups.</p>

				   <p>Your credentials is as follows</p>
                   <br>
				   
				   Userid: <b>'.$emailData['email_id'].'</b>
				   <br>
                   Password: <b>'.$emailData['password'].'</b>
                
                   <p>Click here : <a href="http://pms.karmickdev.com?email='.$emailData['email_id'].'">http://pms.karmickdev.com?email='.$emailData['email_id'].'</a></p>
                    
                <p>Warm Regards</p>
                <p><b>IIMCIP Support Team</b></p>
