...
class PortfolioManagerController extends Controller
{
    ...
    public function index()
    {
        ...
        $dataBag['userList'] = Users::whereIn('user_type', [4])->orderBy('id', 'desc')->get();
            ...
            foreach ($dataBag['userList'] as $user) {
                ...
                if ($count != 0) {
                    $mentor_name = DB::table('users')
                        ->select('first_name', 'contact_name', 'member_company')
                        ->where("id", "=", $mentor_id[0]->mentor_id)->first();
                    // dd($mentor_name);
                    $user->mentor = $mentor_name->first_name != '' ? $mentor_name->first_name : $mentor_name->contact_name;
                } else {
                    $user->mentor = "NA";
                }
                //dd($user);
    ...
    public function assignedStartup()
    {

        //$pm_id = $request->input('pmID'); 
        //$ids_array = DB::table('member_pm_rel')->where('member_id', '=', $pm_id)->get()->toArray();
        $dataBag = array();
        $dataBag['GparentMenu'] = 'pmmanagement';
        $dataBag['parentMenu'] = 'pmManagement';
        $dataBag['childMenu'] = 'pmList';
        //$dataBag['userList'] = $userList= Users::where('founder_id')->whereIn('user_type', [4])->orderBy('id', 'desc')->get();
        //$dataBag['userList'] = $userList= Users::whereIn('user_type', [4])->orderBy('id', 'desc')->get();
        //$dataBag['startupList'] =  Users::where('founder_id')->whereIn('user_type', [4])->orderBy('member_company', 'asc')->get();

        /*$dataBag = DB::table('member_pm_rel')

        -rightjoin('users', 'member_pm_rel.member_id', '=', 'users.id' )
		->select( 'users.*', 'member_pm_rel.pm_id')
		//->from('users', 'member_pm_rel')
		->orderBy('users.member_company', 'asc')
		->where('users.id', '=', Auth::user()->id)
        ->get();
		dd($dataBag);*/
        //return view('dashboard.pm.index', $dataBag);

        $dataBag = Users::join('member_pm_rel', 'member_pm_rel.member_id', '=', 'users.id')
            ->select('users.*')
        ...
        return view('dashboard.pm.startup', ["dataBag" => $dataBag]);
        //dd( $dataBag);
